...
class mybalance_new(iouonegirlPlugin):
    def __init__(self):
    ...
    def balance_before_start(self, roundnumber, direct=False):
        ...
        def try_add_to_queue(p):
            if 'queue' in minqlx.Plugin._loaded_plugins:
                ...
                # log the error to server logs, not console
                minqlx.console_command("echo Unable to add player {} to queue - no queue plugin installed".format(p.name))
                return False
