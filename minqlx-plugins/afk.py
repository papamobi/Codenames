...
class afk(minqlx.Plugin):
    ...
    # Pacing only - the scan needs the main thread, see scan_for_afk().
    @minqlx.thread
    def afk_thread(self):
        if self.running:
    ...
    @minqlx.next_frame
    def scan_for_afk(self):
        # Re-check - the game can have ended since the worker's check.
        if not self.running or not self.game or self.game.state != "in_progress":
            return
    ...
    def spectate_player(self, player):
        ...
        player.spectate()
        self.positions.pop(player.steam_id(), None)
