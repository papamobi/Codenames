...
VERSION = "8.4"
# v8.4: Fixed three critical race conditions found in a minqlx-plugin-review audit,
# all stemming from the same root cause -- using shared self.X instance attributes
# as an inter-function communication channel across concurrent @minqlx.thread
# command handlers, instead of local return values/parameters:
#   1. find_sound_trigger() used to set self.trigger/self.soundFile, read back by
#      scan_chat() -- under concurrent chat from multiple players, one player's
#      lookup could overwrite another's in-flight result, causing the wrong sound
#      to play. Now returns (trigger, soundFile) locally; no shared state.
#   2. Bare "!off"/"!on" (no sound name) read the same self.trigger/self.soundFile
#      to mean "the sound I just triggered" -- same race, but wrote a WRONG,
#      PERSISTENT per-player DB mute/unmute entry rather than just playing the
#      wrong sound transiently. Now uses a per-player self.last_triggered dict.
#   3. exec_sound()'s !playsound file-existence check used shared
#      self.checking_file/self.file_status, read back via the console_print hook --
#      concurrent admin !playsound calls could interleave and corrupt each other's
#      result. Now serialized with a lock. Also fixed the retry loop, which was an
#      `if` instead of a `while` and therefore only ever waited once (0.1s)
#      regardless of the up-to-10-iteration intent.
...
class myFun(minqlx.Plugin):
    ...
    def __init__(self):
        ...
        # serializes concurrent !playsound file-existence checks, since
        # checking_file/file_status are shared state read back via the
        # console_print hook (only one fdir round-trip can be in flight at a time)
        self._file_check_lock = threading.Lock()
        # sounds count
        ...
        self.admin_limiting = {}
        # Dictionary used to store each player's most recently chat-triggered sound,
        # as (trigger, soundFile), for the bare "!off"/"!on" commands. Keyed per
        # player (steam_id) rather than shared, since a single shared self.trigger/
        # self.soundFile previously let one player's chat message overwrite another
        # player's in-flight !off/!on target.
        self.last_triggered = {}
    ...
    def scan_chat(self, player, msg, channel):
        ...
        # find the sound trigger for this sound -- local result, not shared state
        trigger, sound_file = self.find_sound_trigger(self.clean_text(msg))
        if trigger:
            # remember this player's own last-triggered sound, for bare !off/!on
            self.last_triggered[player.steam_id] = (trigger, sound_file)
            # stop sound processing if the player has this sound trigger turned off
            if self.db.exists(PLAYERS_SOUNDS.format(player.steam_id, sound_file)):
            ...
            if delay_time:
                ...
                # otherwise, impose the delay time wait and stop sound trigger processing
                    ...
                    self.tell_main_thread(player, "^3You played a sound recently. {} seconds timeout remaining."
                                                  .format(delay_time))
                    return minqlx.RET_NONE
            ...
            # Make sure the last sound played was not within the sound delay limit
                ...
                self.tell_main_thread(player, "^3A sound has been played in last {} seconds. Try again after the timeout."
                                              .format(self.get_cvar("qlx_funSoundDelay")))
                return minqlx.RET_NONE
            # play_sound runs on the next frame and records the rate limit itself if it dispatches,
            # since it can no longer hand a result back to this worker thread.
            self.play_sound(sound_file, limit_steam_id=player.steam_id)
    ...
    # Returns (trigger, soundFile) tuple on match, (None, None) on no match.
    # IMPORTANT: this must stay free of self.trigger/self.soundFile writes -- it's
    # called concurrently from multiple @minqlx.thread command handlers (chat
    # scanning, !off, !on), and writing a shared result here previously caused a
    # race where one player's lookup could overwrite another's in-flight result.
                ...
                if self._enable_dictionary:
                    if msg_lower[0] in self._sound_dictionary[sound_dict]:
                        for sound, item in self._sound_dictionary[sound_dict][msg_lower[0]].items():
                            match = item.split(";")
                            try:
                                # if sound trigger matches, return (trigger, soundFile)
                                if self.match_string(match[0], match_msg):
                                    if self.db.exists(DISABLED_SOUNDS.format(match[1])):
                                        return None, None
                                    trigger = sound
                                    sound_file = match[1] if match[1] else None
                                    return trigger, sound_file
                            except Exception as e:
                    ...
                    for key in keys:
                        ...
                        try:
                            ...
                            if self.db.exists(TRIGGERS_LOCATION.format(match[1])):
                                for trigger_item in self.db.lrange(TRIGGERS_LOCATION.format(match[1]), 0, -1):
                                    if trigger_item == match_msg:
                                        if self.db.exists(DISABLED_SOUNDS.format(match[1])):
                                            return None, None
                                        trigger = key.split(":")[-1]
                                        sound_file = match[1] if match[1] else None
    ...
    def cmd_sound_off(self, player, msg):
        if len(msg) < 2:
            last = self.last_triggered.get(player.steam_id)
            if last:
                trigger, sound_file = last
                if self.db.exists(PLAYERS_SOUNDS.format(player.steam_id, sound_file)):
                    ...
                    self.db.set(PLAYERS_SOUNDS.format(player.steam_id, sound_file), 1)
    ...
    def cmd_sound_on(self, player, msg):
        if len(msg) < 2:
            ...
            if last:
                trigger, sound_file = last
                if self.db.exists(PLAYERS_SOUNDS.format(player.steam_id, sound_file)):
                    del self.db[PLAYERS_SOUNDS.format(player.steam_id, sound_file)]
    ...
    def exec_sound(self, player, msg, channel):
        if len(msg) < 2:
            self.tell_main_thread(player, "^3Include a path/sound to play.")
            return

        if "console" != channel and not self.db.get_flag(player, "essentials:sounds_enabled", default=True):
            self.tell_main_thread(player, "^1Your sounds are disabled. Use ^6{}sounds ^1to enable them again."
                                          .format(self._command_prefix))
        ...
        if stop is not False:
            ...
            self.tell_main_thread(player, "^3You have {} seconds before you can call another sound.".format(int(stop)))
        else:
            # check for a valid sound file. Serialized via _file_check_lock since
            # checking_file/file_status are read back asynchronously through the
            # console_print hook - only one fdir round-trip can be in flight at a
            # time, or a second concurrent admin call would interleave and corrupt
            # both results.
            with self._file_check_lock:
                self.checking_file = True
                self.file_status = False
                self.run_console_command("fdir {}".format(msg[1]))
                count = 0
                # NOTE: this was previously an `if`, not a `while`, so it only ever
                # waited once (0.1s) regardless of count - the up-to-1-second wait
                # implied by "count < 10" never actually happened. Fixed to while.
                while count < 10 and not self.file_status:
                    count += 1
                    time.sleep(0.1)
                self.checking_file = False
                file_ok = self.file_status
            if not file_ok:
                self.tell_main_thread(player, "^1Sound ^4{} ^1is not valid.".format(msg[1]))
                return
    ...
    def play_trigger(self, player, msg, channel):
        if len(msg) < 2:
            self.tell_main_thread(player, "Include a sound trigger.")
            return

        msg_lower = " ".join(msg[1:]).lower()

        if "console" != channel and not self.db.get_flag(player, "essentials:sounds_enabled", default=True):
            self.tell_main_thread(player, "Your sounds are disabled. Use ^6{}sounds^7 to enable them again."
                                          .format(self._command_prefix))
            return

        sound = self.get_sound_trigger(msg_lower)

        # Same validity test Plugin.play_sound applies, done here so we don't need its return value.
        if not sound or "music/" in sound.lower():
            self.tell_main_thread(player, "^1Invalid sound.")
            return

        if "console" == channel:
            minqlx.console_print("^1Playing sound^7: ^4{}".format(msg_lower))
        else:
            # Local preview for the caller; send_server_command needs the main thread.
            self.play_sound_for(sound, player)

        self.play_sound(sound)

        return

    # Main thread only - send_server_command; called from the play_trigger worker.
    @minqlx.next_frame
    def play_sound_for(self, _path, player):
        super().play_sound(_path, player)

    # Main thread only - Cmd_ExecuteString; called from the exec_sound worker.
    @minqlx.next_frame
    def run_console_command(self, command):
        minqlx.console_command(command)

    # player.tell() for use from a @minqlx.thread worker. Plugin.tell -> TellChannel.reply ->
    # players()/send_server_command, all of which read or write engine state.
    @minqlx.next_frame
    def tell_main_thread(self, player, message):
        player.tell(message)
    ...
    # Main thread only - players()/teams()/play_sound() read the engine's client array, and every
    # caller (scan_chat, exec_sound, play_trigger) is a @minqlx.thread worker.
    # The next_frame wrapper always returns None, so the old "did the sound play?" return value is
    # gone; pass limit_steam_id and this records the per-player rate limit itself instead.
    @minqlx.next_frame
    def play_sound(self, _path, player=None, limit_steam_id=None):
        # Delayed to the next frame, so the game may be gone by now.
        active = self.game is not None and self.game.state in ["in_progress", "countdown"]
        play = self.last_2_sound(active)

        if play == 3 or not active:
            recipients = self.players()
        elif play == 1 or (play == 2 and active):
            teams = self.teams()
            recipients = [p for p in teams["red"] + teams["blue"] + teams["free"] if not p.is_alive]
            recipients += teams["spectator"]
        else:
            return
        ...
        if limit_steam_id is not None:
            self.sound_limiting[limit_steam_id] = time.time()

    # Main thread only - called from play_sound(), which is @minqlx.next_frame.
    def last_2_sound(self, active=True):
        # 0 = don't play sound for anyone when the last 2 (or  1 on either team of a team based game) remains
        # 1 = play sound for all except those alive when the last 2 (or  1 on either team of a team based game) remains
        # 2 = only play sounds for people who are dead/spectating when game is active
        # 3 = play sound for everyone with sounds enabled
    ...
    def cmd_help(self, player, msg=None, channel=None):
        ...
        player.tell("^1myFun Version {0}\n"
                    ...
                    "^6{1}listsounds all ^3shows every sound in every enabled pack\n"
                    ...
                    " ^4Default Quake Live Sounds\n^6{1}listsounds #Default yeah ^3 will search ^4#Default"
    ...
    def listsounds_usage(self, player):
        if self._total_sounds:
            ...
            player.tell("^1There are {2} sounds available to play.\n"
                        ...
                        "^5Ex: ^4{0}listsounds all ^7(shows every sound in every enabled pack)\n"
                        "^3Valid sound-pack(s) are^7:\n^2{1}\n"
    ...
    def cmd_list_sounds(self, player, msg, channel):
        ...
        category = None
        search = None
        # Tokens that mean "list everything across all enabled packs".
        all_tokens = ("all", "*", "#all", "#*")
        if length == 2 and msg[1]:
            if msg[1].lower() in all_tokens:
                pass  # category=None, search=None -> falls through to list-all path
