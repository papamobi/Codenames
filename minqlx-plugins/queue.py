...
class queue(minqlx.Plugin):
    ...
    def pushFromQueue(self, delay=0):
                    ...
                    if self._queue[1] in spectators and self._queue[1].connection_state == 'active':
                        ...
                        if 'balance' in minqlx.Plugin._loaded_plugins:
                            try:
                                ...
                                minqlx.console_print("Unable to move players to teams according to elo, reason: {}".format(e))
                                p1.put("red")
                                p2.put("blue")
                        else:
                            p1.put("red")
        ...
        @minqlx.next_frame
        def checkForPlace():
            # Clears is_push_pending on every exit path, so the push stays debounced.
            try:
                # Re-check - is_endscreen can have flipped during the sleep.
                if self.is_endscreen or self.game is None:
                    return
        ...
        # Flag handed to checkForPlace() only if we get that far; released here otherwise.
        ready = False
        try:
            time.sleep(delay)
            ...
            if self.is_endscreen:
                return
            game = self.game
            if game is None or game.state not in ('in_progress', 'warmup'):
                return
            ready = True
        finally:
            if not ready:
