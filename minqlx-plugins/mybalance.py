...
# This code makes sure the required superclass is loaded automatically. If
# it's missing, we try to fetch it from the canonical upstream, but only
# with strict safety limits: bounded timeout, size check, content sanity
# check, and an atomic write via tempfile + rename. Any failure raises a
# clear ImportError telling the admin how to install it manually.
try:
    from .iouonegirl import iouonegirlPlugin
except ImportError:
    _IOUONEGIRL_URL = (
        "https://raw.githubusercontent.com/dsverdlo/minqlx-plugins/"
        "master/iouonegirl.py"
    )
    _IOUONEGIRL_MANUAL_INSTRUCTIONS = (
        "mybalance requires iouonegirl.py in the plugins directory. "
        "Download it manually from {} and place it next to mybalance.py."
    ).format(_IOUONEGIRL_URL)

    def _download_iouonegirl():
        """Fetch iouonegirl.py with bounded timeout, sanity-check the
        content, and write atomically. Raises on any problem."""
        import tempfile
        # Bounded timeout: 5s to connect, 30s total. Blocks import, so
        # keep it short enough that a slow GitHub doesn't stall the server.
        res = requests.get(_IOUONEGIRL_URL, timeout=(5, 30))
        res.raise_for_status()

        text = res.text
        # Size sanity: too small = probably an error page, too large = fishy.
        if not (1000 <= len(text) <= 200000):
            raise ValueError(
                "iouonegirl.py fetched but size looks wrong ({} bytes)"
                .format(len(text))
            )
        # Content sanity: must look like a Python module that defines the
        # expected class. Cheap defense against being served an HTML error
        # page or a redirect.
        if "class iouonegirlPlugin" not in text:
            raise ValueError(
                "iouonegirl.py fetched but doesn't contain the expected class"
            )

        target_path = os.path.join(os.path.dirname(__file__), "iouonegirl.py")
        # Write to a tempfile in the same directory (so the rename is atomic
        # on POSIX) then rename over the target. Prevents leaving a
        # half-written .py file if we die mid-write.
        tmp_fd, tmp_path = tempfile.mkstemp(
            prefix="iouonegirl.", suffix=".tmp",
        ...
        try:
            ...
            # Best-effort cleanup so we don't leave orphaned tmp files.
            try:
                os.unlink(tmp_path)
            except OSError:
                pass
    ...
    try:
        _download_iouonegirl()
        from .iouonegirl import iouonegirlPlugin
        minqlx.get_logger().info(
            "mybalance: downloaded iouonegirl.py from upstream"
        )
    except Exception as e:
        minqlx.get_logger().error(
            "mybalance: iouonegirl auto-install failed (%s). %s",
            e, _IOUONEGIRL_MANUAL_INSTRUCTIONS,
        )
        raise ImportError(_IOUONEGIRL_MANUAL_INSTRUCTIONS) from e
...
class mybalance(iouonegirlPlugin):
    ...
    def handle_new_game(self):
        # Delayed 3s, so the game may be gone by now.
    ...
    def handle_player_connect(self, player):
        ...
        if not (player.steam_id in self.exceptions or self.db.has_permission(player, self.get_cvar("qlx_mybalance_perm_allowed", int))):
            ...
            try:
                ...
                minqlx.console_print("MybalanceError: {}".format(e))
    ...
    def balance_before_start(self, context, direct=False):
        ...
        state = {"excluded": None, "done": False}
        for _ in range(20):
    ...
    def balance_step(self, context, state):
        ...
        if gametype in ("ca", "ft") and self.game.roundlimit in [self.game.blue_score, self.game.red_score]:
            state["done"] = True
            return
        if gametype == "tdm" and self.game.fraglimit in [self.game.blue_score, self.game.red_score]:
            state["done"] = True
            return
        if gametype == "ctf" and self.game.capturelimit in [self.game.blue_score, self.game.red_score]:
            state["done"] = True
            return

        # Exclude the prevented/ignored player from this frame's snapshot, not a stale one.
        if state["excluded"] is not None:
            red = [p for p in red if p.steam_id != state["excluded"]]
            blue = [p for p in blue if p.steam_id != state["excluded"]]
...
##                self.balance_before_start(round_number, True)
    ...
    def algo_get_last(self, teams=None):
        ...
        minqlx.console_command("echo Current join times: {}".format(self.join_match_times))
    ...
    def fetch(self, player, gt, callback):
        ...
        while attempts < MAX_ATTEMPTS:
            ...
            try:
                res = requests.get(url, timeout=(2.0, 5.0))
            except requests.RequestException as e:
                # Timeout, connection error, etc. -- fall through to retry
                # loop rather than let the worker die silently.
                last_status = "network error: {}".format(e)
                continue
            ...
            for p in js["players"]:
                ...
                if _sid == sid: # got our player
                    if gt not in p:
                        # console_print, not console_command -- Cmd_ExecuteString needs the main
                        # thread and this runs on the fetch() worker.
                        minqlx.console_print("No {} rating for {}".format(gt, _sid))
        ...
        self.msg_main_thread("^1echo Problem fetching {} glicko: {}".format(gt, last_status))
        return
    ...
    def fetch_both(self, player, gt, channel):
        ...
        try:
            ...
            # msg(), not channel.reply() - worker thread.
            msg(elomsg)

        except Exception as e:
            msg("^1Problem fetching {} glicko: {}".format(gt, e))
            raise e
            return
...
class ConnectThread(threading.Thread):
    ...
    def run(self):
        ...
        self._result = requests.get(url, timeout=(2.0, 5.0))